...
const COMMAND = [
  ...
  '--connect',
  '--output-json',
  '--command=/subsystem=datasources/xa-data-source=cis4c-be-pool:read-resource(include-runtime=true,recursive=true,resolve-expressions=true)'
];
...
function extractJson(output) {
  ...
  if (start === -1 || end === -1) throw new Error('JSON non trovato nell’output');
...
async function saveStatsToDB(podName, stats) {
  ...
  const insertQuery = `INSERT INTO cis4c_delivery.pool_stats (execution_id, namespace, pod_name, active_count, available_count, idle_count, in_use_count, max_pool_size, connectable)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`;
  const values = [EXECUTION_ID, NAMESPACE, podName, stats.ActiveCount, stats.AvailableCount, stats.IdleCount, stats.InUseCount, stats.MaxPoolSize, stats.Connectable];
...
function execCommandOnPod(podName) {
  ...
  try {
    ...
    stats = {
      ...
      Connectable: jsonOutput.result.connectable || false
    };

    if (!stats.Connectable) {
      console.warn(`⚠️ Datasource non connesso: statistiche potrebbero essere incomplete`);
    }
  } catch (err) {
...
const requiredVars = ['NAMESPACE','OC_SERVER','OC_USERNAME','OC_PASSWORD','PG_HOST','PG_PORT','PG_USER','PG_PASSWORD','PG_DATABASE','NODE_INTERVAL_CRON'];
...
if (missingVars.length > 0) {
  ...
  setInterval(() => { console.log('In attesa che le variabili siano definite...'); }, 60000);
  return;
}
